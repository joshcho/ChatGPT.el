...
- If you get a ~deferred error~ with "Browser closed" messages, try closing all firefox instances by doing "pkill firefox" in the shell. Then M-x chatgpt-stop, followed by M-x chatgpt-query (or ~C-c q~).
